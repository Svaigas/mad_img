...
function metadataHandler(request, reply) {
	...
	for (var i in request.payload.size) {
		request.payload.size[i] = JSON.parse(request.payload.size[i]);
	}

	return resizeImages(id, request.payload.data, request.payload.size)
		.then(uploadDB)
		.then(reply)
		.catch((error) => {
			return reply(error)
